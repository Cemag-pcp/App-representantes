var btnSalvarContato = document.getElementById('salvarCadastroContato_1');

btnSalvarContato.addEventListener('click', function(){

    var idEmpresa = document.getElementById('empresaCadastrada').dataset.id;
    var nomeContato = document.getElementById('nomeContato');
    var telefoneContato = document.getElementById('telefoneContato');

    criarContato(nomeContato.value,idEmpresa,telefoneContato.value);
    
});

document.addEventListener('DOMContentLoaded', (event) => {
    
    var idEmpresa = document.getElementById('empresaCadastrada');
    var nomeContato = document.getElementById('nomeContato');
    var telefoneContato = document.getElementById('telefoneContato');
    var inputCidadeContato = document.getElementById('inputCidadeContato');

    function checkInputs() {
        if (
            idEmpresa.value.trim() !== '' &&
            nomeContato.value.trim() !== '' &&
            telefoneContato.value.trim() !== '' &&
            inputCidadeContato.value.trim() !== ''
        ) {
    ...
    idEmpresa.addEventListener('input', checkInputs);
    nomeContato.addEventListener('input', checkInputs);
    telefoneContato.addEventListener('input', checkInputs);
    inputCidadeContato.addEventListener('input', checkInputs);
});

function criarContato(nomeContato,idEmpresa,telefone) {

    showSpinner();

    var typePhone = document.getElementById('typePhone2').value;
    ...
    fetch('https://api2.ploomes.com/Contacts?', options)
...
//             showSuggestions1_contatos(contatos.value);
//         })
//         .catch(error => {
