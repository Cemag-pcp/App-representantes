...
var btContinuarCarrinho = document.getElementById('continuarConfirmacao');
...
btAbrirCarrinho.addEventListener('click', function () {
    ...
    if (nomeCliente && formaPagamento) {
        ...
        modalBody.innerHTML = '';
        modalBody.innerHTML = '<p>Preencha o nome do Cliente e Forma de pagamento</p>';
    };
    ...
    btContinuarCarrinho.addEventListener('click', function () {
        ...
        if (nomeCliente && formaPagamento) {
            ...
            modalBodyContinuar.innerHTML = '';
            modalBodyContinuar.innerHTML = '<p>Preencha o nome do Cliente e Forma de pagamento</p>';
        };
...
function preencherModalProduto(id, produto, cor, quantidade, preco, precoInicial, desconto) {
    ...
    desconto.setAttribute('id', 'desconto');
    desconto.setAttribute('type', 'text');
    desconto.setAttribute('class', 'form-control');
    desconto.setAttribute('value', desconto);
    ...
    colDivPrecoInicial.classList.add('col-md-1');
...
function contiuarCarrinho() {
    ...
    newRow.innerHTML = `
        <div class="col-md-4"><strong>Revenda:</strong> <span style="background-color:yellow">${nomeCliente}</span></div>
        <div class="col-md-4"><strong>Contato:</strong> <span style="background-color:yellow">${nomeContato}</span></div>
        <div class="col-md-4"><strong>Forma de pagamento:</strong> <span style="background-color:yellow">${formaPagamento}</span></div>
    `;
    ...
    rows.forEach(function (row, index) {
        ...
        var preco = parseFloat(row.querySelector('#precoUnitItem').value);
        ...
        // Preenche o novo elemento div com as informações do item
        newRow.innerHTML = `
            <div class="col-md-6" id="produto-${index}"><strong>Produto:</strong> <span style="background-color:yellow">${produto}</span></div>
            <div class="col-md-2" id="cor-${index}"><strong>Cor:</strong> <span style="background-color:yellow">${cor}</span></div>
            <div class="col-md-2" id="quantidade-${index}"><strong>Quantidade:</strong> <span style="background-color:yellow">${quantidade}</span></div>
            <div class="col-md-2" id="preco-${index}"><strong>Preço:</strong> <span style="background-color:yellow">${preco}</span></div>
    ...
    // Preenche o novo elemento div com as informações do item
    var valorTotalNumero = parseFloat(valorTotal);

    if (!isNaN(valorTotalNumero)) {
        // Formate o valor total para exibir o símbolo "R$" com separadores de milhar e decimal
        var valorTotalFormatado = valorTotalNumero.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

        newRow.innerHTML = `
            <div class="col-md-12"><strong>Total:</strong> ${valorTotalFormatado}</div>
        `;
    } else {
        console.error('O valor total não é um número válido.');
    }
